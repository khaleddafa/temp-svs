...
class ClubController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data['items'])
                ...
                ->addColumn('bonus', function ($item) {
                    return user_plan_bonus($item->user_id);
                })
    ...
    public function clubBonusDistribution(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data['items'])
                ...
                ->addColumn('bonus_coin_type', function ($item) {
                    return check_default_coin_type($item->bonus_coin_type);
                })
                ->make(true);
        }
    ...
    public function planList(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($data['items'])
                ...
                ->addColumn('bonus_coin_type', function ($item) {
                    return check_default_coin_type(DEFAULT_COIN_TYPE);
                })
    ...
    {
        $data['title'] = __('Add New Plan');
        $data['coins'] = Coin::where(['status' => STATUS_ACTIVE, 'type' => DEFAULT_COIN_TYPE])->get();
    ...
    {
        $data['title'] = __('Membership Settings');
        $data['settings'] = allsetting();

        return view('admin.club.settings', $data);
    }
}
